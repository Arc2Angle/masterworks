...
import com.masterworks.masterworks.init.MasterworksRecipeSerializers;
import com.masterworks.masterworks.init.MasterworksRecipeTypes;
...
public record ConstructionRecipe(Ingredient template, List<Ingredient> components)
    ...
    public boolean matches(@Nonnull ConstructionRecipeInput input, @Nonnull Level level) {
        return template.test(input.template())
                && BiStream.zip(components.stream(), input.components().stream())
                        .map(Ingredient::test).allMatch(b -> b);
    }

    @Override
    public ItemStack assemble(@Nonnull ConstructionRecipeInput input,
    ...
    public PlacementInfo placementInfo() {
        return PlacementInfo
                .create(Stream.concat(Stream.of(template), components.stream()).toList());
    }
    ...
    public RecipeBookCategory recipeBookCategory() {
        throw new UnsupportedOperationException(
                "Construction Recipe does not appear in the recipe book");
    }
