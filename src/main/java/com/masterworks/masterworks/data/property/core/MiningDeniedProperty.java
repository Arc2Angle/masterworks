...
public sealed interface MiningDeniedProperty extends ToolRuleProperty
    ...
    default Tool.Rule getToolRule(Construct construct) {
        ...
        HolderSet.Named<Block> set = BuiltInRegistries.BLOCK.get(tagKey)
    ...
    public static final class Type implements ToolRuleProperty.Type<MiningDeniedProperty> {
        @Override
        public Decoder<MiningDeniedProperty> decoder(
            ...
            return Decoder.ofSimple(new Decoder.Simple<>() {
                ...
                public <T> DataResult<MiningDeniedProperty> decode(Dynamic<T> input) {
                    return Codec.STRING.parse(input).flatMap(argument -> {
                        if (!argument.startsWith(ExpressionProperty.ARGUMENT_PREFIX)) {
                            return DataResult.error(
                                    () -> "Invalid mining denied argument (must be prefixed with $): "
                                            + argument);
                        }
                    ...
                    }, error -> TagKey.codec(Registries.BLOCK).parse(input).flatMap(tagKey -> {
    ...
    record Reference(Construct.Component.Key key, RoleReferenceLocation role)
            implements MiningDeniedProperty {
        @Override
        public TagKey<Block> blocks(Map<Construct.Component.Key, Construct.Component> components) {
            ...
            MiningDeniedProperty property =
                    component.properties(role).get(type()).orElseThrow(() -> new RuntimeException(
                            "Property " + type() + " not found in component " + key));
